    ...
    def do(self, dispatcher, monitor):
        ...
        if driver is not None:
            ...
            events.append(Pickup(self.timestamp + travel_time,
                                 self.rider, driver))
        events.append(Cancellation(self.timestamp +
                                   self.rider.patience, self.rider))
        return events
...
class DriverRequest(Event):
    ...
    def do(self, dispatcher, monitor):
        ...
        #DO EXAMPLES OR NOT SINCE ABOVE DID NOT
        """
        events = []
        monitor.notify(self.timestamp, DRIVER, REQUEST, self.driver.id, self.driver.location)
        rider = dispatcher.request_rider(self.driver)
        if rider is not None:
            dispatcher.cancel_ride(rider)
            self.driver.start_drive(rider.origin)
            events.append(Pickup(self.timestamp + self.driver.get_travel_time(rider.origin), rider, self.driver))
        return events
...
class Cancellation(Event):
    ...
    def do(self, dispatcher, monitor):
        """Cancel the request of the rider if he/she has not
    ...
    def __str__(self):
        ...
        #DO EXAMPLES OR NOT SINCE ABOVE DID NOT
        """
        return "{} -- {}: Cancel a request".format(self.timestamp, self.rider)
...
class Pickup(Event):
    ...
    def do(self, dispatcher, monitor):
        """Pick up the rider is he/she is waiting or request a new
        rider for the driver if the rider cancelled their request.
        ...
        if self.rider.status == WAITING:
            ...
            events.append(Dropoff(self.timestamp + self.driver.get_travel_time(self.rider.destination),
                                  self.rider, self.driver))
            self.rider.status = SATISFIED
        elif self.rider.status == CANCELLED:
            monitor.notify(self.timestamp, DRIVER, CANCEL, self.driver.id, self.driver.location)
...
class Dropoff(Event):
    ...
    def do(self, dispatcher, monitor):
        """Drop off the rider at their destination and sets the rider's status to satisfied.

        #IS DISPATCHER EVEN USED?!?!?!?!
        ...
        self.rider.status = SATISFIED
        monitor.notify(self.timestamp, DRIVER, DROPOFF, self.driver.id, self.driver.location)
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                driver = Driver(tokens[2], deserialize_location(tokens[3]), int(tokens[4]))
                # Create a DriverRequest event.
                event = DriverRequest(timestamp, driver)
            elif event_type == "RiderRequest":
                rider = Rider(tokens[2], deserialize_location(tokens[3]), deserialize_location(tokens[4]),
                              WAITING, int(tokens[5]))
